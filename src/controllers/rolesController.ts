...
export async function getAllRoles(_req: Request,res: Response,next: NextFunction) {
  try {
...
export async function updateRoles(req: Request,res: Response,next: NextFunction) {
  try {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) {
      return next({
    ...
    const duplicate = await db
      .select()
      .from(roles)
      .where(and(eq(roles.role_name, role_name), ne(roles.id, id)))
      .limit(1);
    if (duplicate[0]) {
      return next({
        statusCode: 409,
...
export async function removeRoles(req: Request,res: Response,next: NextFunction) {
  try {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) {
      return next({
