...
export const checkIn = async (req, res, next) => {
    try {
        ...
        const existingLog = await db
            .select()
            .from(attendanceLogs)
            .where(and(eq(attendanceLogs.employeeId, employeeId), eq(attendanceLogs.attendanceDate, todayStr)))
            .limit(1);
...
export const getAllAttendanceLogs = async (_req, res, next) => {
    try {
        const logs = await db
            .select({
...
export const getMonthlyAttendanceLogs = async (req, res, next) => {
    try {
        ...
        res.setHeader("Content-Disposition", `attachment; filename=Attendance-${numericMonth}-${numericYear}.xlsx`);
...
export const getDepartmentMonthlyReport = async (req, res, next) => {
    try {
        ...
        const rawData = await db
            ...
            .leftJoin(moodJournals, and(eq(attendanceLogs.employeeId, moodJournals.employeeId), gte(moodJournals.createdAt, startDateObj), lte(moodJournals.createdAt, endDateObj)))
            .where(and(gte(attendanceLogs.attendanceDate, startDateStr), lte(attendanceLogs.attendanceDate, endDateStr)));
        const workbook = new ExcelJS.Workbook();
        ...
        res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        res.setHeader("Content-Disposition", `attachment; filename=DepartmentReport-${numericMonth}-${numericYear}.xlsx`);
        await workbook.xlsx.write(res);
        res.end();
        return;
    }
...
//# sourceMappingURL=attedanceLogsController.js.map