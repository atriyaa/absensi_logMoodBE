...
export const getAdminDashboard = async (req, res, next) => {
    try {
        ...
        const [totalEmployees, totalDepartments, attendanceToday, moodToday, recentAttendance] = await Promise.all([
            db.select({
                total: count()
            }).from(employees),
            db.select({
            ...
            db.select({
                ...
                .leftJoin(moodJournals, eq(moodJournals.attendanceLogId, attendanceLogs.id))
                .where(eq(attendanceLogs.attendanceDate, today))
        ...
        const percentage = totalEmployee === 0
            ? 0
